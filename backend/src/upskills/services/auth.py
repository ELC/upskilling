...
class AuthService:
    ...
    async def register(
        ...
        user = await self.user_repository.get_by_id_with_roles(db_model.user_id)
        tokens = self.create_tokens(db_model.user_id)

        return AuthResult(user=user, tokens=tokens)

    async def login(self, email: str, password: str) -> AuthResult:
        db_model = await self.user_repository.get_by_email(email)

        if not db_model or not verify_password(password, db_model.password_hash):
            msg = "Invalid email or password"
            raise ValueError(msg)

        user = self.user_repository.to_domain(db_model)
        tokens = self.create_tokens(db_model.user_id)
    ...
    async def refresh_tokens(self, refresh_token: str) -> Token:
        ...
        return self.create_tokens(db_model.user_id)
    ...
    async def request_password_reset(self, email: str) -> str | None:
        ...
        return await self.user_repository.create_password_reset_token(db_model.user_id)
    ...
    async def reset_password(self, token: str, new_password: str) -> bool:
        ...
        db_model = await self.user_repository.get_by_id(reset_token.user_id, id_column="user_id")
        if db_model:
